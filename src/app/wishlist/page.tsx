...
import { WishlistShareButton } from "@/features/account/components/wishlist-share-button";
...
export default async function WishlistPage() {
  ...
  const [wishlist, shareToken] = await Promise.all([
    WishlistService.getUserWishlist(user.id) as Promise<{ items: unknown[] }>,
    WishlistService.getWishlistShareToken(user.id),
  ]);
  ...
  return (
    <Section className="py-macro-md md:py-macro-lg bg-white min-h-screen">
      <Container className="max-w-container-max">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-macro-sm md:mb-macro-md gap-4">
          ...
          {products.length > 0 && (
            <WishlistShareButton existingToken={shareToken} />
          )}
        </div>
